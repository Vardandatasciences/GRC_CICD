...
LOGGING = {
    ...
    'loggers': {
        ...
        # Reduce noise from authentication and request logs while keeping errors visible
        'grc.authentication': {
            'handlers': ['console'],
            'level': 'ERROR',  # Only show actual errors, not INFO about rate limits
            'propagate': False,
        },
        'django.request': {
            'handlers': ['console'],
            'level': 'ERROR',  # Only log 5xx errors; completely hide 401/403/429 warnings
            'propagate': False,
        },
        'django.server': {
            'handlers': ['console'],
            'level': 'ERROR',  # Only log real server errors, hide 4xx warnings from basehttp
            'propagate': False,
        },
    },
}
